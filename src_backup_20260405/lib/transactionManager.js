...
export class TransactionManager {
  ...
  static async addTransaction(transactionData) {
    ...
    console.log('TransactionManager 추가 시도:', { userId, transactionData });
    ...
    console.log('TransactionManager 추가 결과:', { data, error });
  ...
  static async updateTransaction(transactionId, updateData) {
    ...
    console.log('TransactionManager 수정 시도:', { transactionId, userId, updateData });
    ...
    console.log('TransactionManager 수정 결과:', { data, error });
  ...
  static async addStock(stockData) {
    ...
    console.log('종목 추가 데이터:', stockData);
    ...
    try {
      ...
      // 마지막 틱은 항상 유지 (중첩될 경우 이전 것을 제거하고 마지막을 강제 고정)
      if (t === sortedRawTicks[sortedRawTicks.length - 1]) {
        if (diff < (minGap * 0.8) && filtered.length > 1) {
          filtered.pop();
        }
        filtered.push(t);
      } else if (diff >= minGap) {
    ...
    try {
      ...
      if (toCurrency === 'KRW') {
        ...
        if (dbData && dbData.rate) {
          console.log(`[DB] 환율 조회 성공 (${date}, ${fromCurrency}):`, dbData.rate);
          return dbData.rate;
        }
        ...
        if (dbError && Object.keys(dbError).length > 0) {
          ...
          console.log(`[DB] 해당 일자/통화의 환율 데이터가 없음: ${date}, ${fromCurrency}`);
        }
      }
